...
| `requirements-deploy-rag.txt` | Docker API 镜像 + RAG/长期记忆（`API_REQUIREMENTS_FILE` 选用） |
| `requirements-serve-rag.txt` | 本地 API + RAG（在 `requirements-serve.txt` 之上） |
| `requirements-rag.txt` | ChromaDB、sentence-transformers、BGE 等 RAG 可选依赖 |
| `requirements-full.txt` | 本地研发全量依赖 |

### 4. 启用 RAG（检索增强生成）

项目已实现基于 **ChromaDB** 的向量检索与 **BGE-Reranker** 精排，用于长期记忆召回、三重校验证据检索及决策工作流 `memory_recall` 节点。默认在 `config.yaml` 中开启（`harness.memory.long_term.rag.enabled: true`）。

**1. 安装 RAG 依赖**

```bash
# 本地开发（在已安装 requirements-serve.txt 与 workspace 包之后）
uv pip install -r requirements-serve-rag.txt
# 或一次性全量：uv pip install -r requirements-full.txt

# Docker 部署需将 .env 中 API_REQUIREMENTS_FILE 改为：
# API_REQUIREMENTS_FILE=requirements-deploy-rag.txt
```

**2. 准备知识库 Markdown**

文档目录：`knowledge_base/`（6 个核心文件 + 场景子目录 `chemical/`、`metallurgy/`、`dust/`）。首次可从公开数据生成：
...
索引持久化到 `var/chroma/`（已在 `.gitignore` 中，需本地或部署时重建）：

```bash
export MINING_PROJECT_ROOT="$(pwd)"

# 推荐：离线/CI 使用 deterministic fallback（无需下载 BGE 模型）
python scripts/rebuild_rag_index.py --clear --embedding-backend fallback

# 生产：使用真实 BGE 嵌入（需已安装 sentence-transformers，首次会下载模型）
python scripts/rebuild_rag_index.py --clear --embedding-backend auto
```
...
```bash
pytest tests/test_vector_store.py::TestFormalRagIndex -q
```
